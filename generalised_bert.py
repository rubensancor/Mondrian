...
parser.add_argument('--train_split', required=True, type=int, help='Train split. Set the percentage for the training set.') 
parser.add_argument('--test_size', default=1.0, type=float, help='Test size. Set the percentage for the testing set.')
...
edited_users_test = [False] * num_users
...
train_end_idx = args.train_split
...
for i in range(len(user_embeddings)):
    if edited_users[i]:
        X_train.append(user_embeddings[i].tolist())
        y_train.append(head_labels[user_list_id.index(i)])
    if edited_users_test[i]:
        X_test.append(user_embeddings[i].tolist())
